...
const graph = async (sheet: SheetJSON, onClick: (graph: GraphJSON, operator: OperatorJSON) => void) => {
  ...
  try {
    ...
    graph.addListener("click", (graph: mxGraph, e: mxEventObject) =>{
      ...
      try {
        model.beginUpdate();
        Object.values(eventNodes).forEach(n => n.connectors.forEach(c => c.setVisible(false)));
        Object.values(stateNodes).forEach(n => n.connectors.forEach(c => c.setVisible(false)));
        Object.values(taskNodes).forEach(n => n.connectors.forEach(c => c.setVisible(false)));
